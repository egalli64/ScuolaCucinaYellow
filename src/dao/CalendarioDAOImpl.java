...
public class CalendarioDAOImpl implements CalendarioDAO {
	...
	public void delete(int idEdizione) throws SQLException {
		...
		if (n == 0) {
			throw new SQLException("L'edizione " + idEdizione + " non esiste!");
		}
	}
	...
	public ArrayList<Edizione> select(java.util.Date da, java.util.Date a) throws SQLException {
		...
		PreparedStatement ps = conn.prepareStatement(
				"SELECT id_edizione FROM calendario where CAST(dataInizio AS DATE) between '?' and '?'");
		ResultSet rs = ps.executeQuery();
		ps.setDate(1, new java.sql.Date(da.getDate()));
		ps.setDate(2, new java.sql.Date(a.getDate()));
	 ...
	 * dei corsi
	 * 
	 * se future = true, le edizioni lette devono essere solo quelle a partire dalla
	 * data odierna e dell'anno corrente
	 * 
	 * se future = false devono essere lette tutte le edizioni le edizioni vengono
	 * individuate in base al idCategoria
	 * 
	 * se non vi sono edizioni per quella categoria o la categoria non esiste viene
	...
	public ArrayList<Edizione> select(int idCategoria, boolean future) throws SQLException {
		...
		ArrayList<Edizione> result = new ArrayList<Edizione>();
		String query = "select id_edizione from categoria join catalogo using (id_categoria) join calendario using (id_corso) where id_categoria=?";
		if (future == true) {
			query += " and dataInizio >= curdate()";
		}
	...
	public ArrayList<Edizione> select(boolean future) throws SQLException {
		ArrayList<Edizione> result = new ArrayList<Edizione>();
		String query = "select id_edizione from categoria join catalogo using (id_categoria) join calendario using (id_corso)";
		if (future == true) {
			query += " where dataInizio >= curdate()";
		}
		PreparedStatement ps = conn.prepareStatement(query);
		ResultSet rs = ps.executeQuery();
		while (rs.next()) {
	...
	public ArrayList<Edizione> select(String idUtente, boolean future) throws SQLException {
		ArrayList<Edizione> result = new ArrayList<Edizione>();
		String query = "select id_edizione from categoria join catalogo using (id_categoria) join calendario using (id_corso) where id_utente=?";
		if (future == true) {
			query += " and dataInizio >= curdate()";
		}
		PreparedStatement ps = conn.prepareStatement(query);
